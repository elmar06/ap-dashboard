...
class Reports
{
    ...
    {
        $query = 'SELECT po_details.id as "po-id", po_details.company, po_details.project, po_details.supplier, po_details.or_num, po_details.po_num, po_details.ir_rr_no, po_details.si_num, po_details.po_amount, po_details.amount, po_details.bill_date, po_details.counter_date, po_details.due_date, po_details.memo_no, po_details.date_submit, po_details.remark, po_details.status FROM po_details WHERE po_details.status != 0 AND (po_details.date_submit BETWEEN ? AND ?)';
        //$query = 'SELECT * FROM po_details WHERE status != 0 AND (date_submit BETWEEN ? AND ?)';
        $this->conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
		$sel = $this->conn->prepare($query);
    ...
    // GET THE PO OTHER DETAILS
    public function get_other_po_details()
    {

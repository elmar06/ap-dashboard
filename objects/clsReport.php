...
class Reports
{
    ...
    {
        $query = 'SELECT po_details.company, po_details.project, po_details.supplier, po_details.or_num, po_details.po_num, po_details.si_num, po_details.amount, po_details.status, po_other_details.date_received_fo, po_other_details.date_to_ea, po_other_details.date_from_ea, po_other_details.date_release, check_details.cv_no, check_details.bank, check_details.check_no, check_details.check_date, check_details.tax, check_details.cv_amount, CONCAT(users.firstname, " ", users.lastname) as "fullname" FROM po_details, po_other_details, check_details, users WHERE po_details.id = po_other_details.po_id AND po_details.id = check_details.po_id AND po_details.submitted_by = users.id AND po_details.project = ? AND po_details.status != 0 AND (po_details.date_submit BETWEEN ? AND ?)';
        $this->conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
		$sel = $this->conn->prepare($query);
    ...
    {
        $query = 'SELECT po_details.id as "po-id", po_details.company, po_details.project, po_details.supplier, po_details.or_num, po_details.po_num, po_details.si_num, po_details.amount, po_details.status, po_other_details.date_received_fo, po_other_details.date_to_ea, po_other_details.date_from_ea, po_other_details.date_release, CONCAT(users.firstname, " ", users.lastname) as "fullname" FROM po_details, po_other_details, users WHERE po_details.submitted_by = users.id AND po_details.id = po_other_details.po_id AND po_details.status = ? AND po_details.status != 0 AND (po_details.date_submit BETWEEN ? AND ?)';
        $this->conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
		$sel = $this->conn->prepare($query);

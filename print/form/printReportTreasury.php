...
$title_name = 'Treasury Report('.date('M d, Y', strtotime($from)).' - '.date('M d, Y', strtotime($to)).')';
...
if($comp_id != null && $comp_id != '')
{
	...
	while($row = $get_data->fetch(PDO::FETCH_ASSOC))
	{
        ...
        <tr>
            ...
            <td align="center" style="border-top-style:2px; border-left-style:2px; border-bottom-style:2px">'.$received_date.'</td>
...
//GENERATE REPORT BY DATE AND SUPPLIER
elseif($supplier_id != null || $supplier_id != '')
{
	$get_data = $report->generate_by_comp_treasury($from, $to, $supplier_id);
	while($row = $get_data->fetch(PDO::FETCH_ASSOC))
	{
		$check_date = date('m/d/y', strtotime($row['check_date']));
		$cv_num = $row['cv_no'];
...
<table width="100%" cellpadding="10" style="border-top-style:2px; border-left-style:2px; border-right-style:2px; border-bottom-style:2px; font-size: 10px">
	<thead>
		<tr>
			...
			<td align="center" style="border-top-style:2px; border-left-style:2px; border-bottom-style:2px"><b>Supplier</b></td>
